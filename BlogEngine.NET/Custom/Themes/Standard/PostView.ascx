...
<script>
    var NeetTeen = NeetTeen || { CustomClass: '', CustomEnum: '' };
    <% if (Post.CustomFields.ContainsKey("CustomClass")) {
        var CustomClass = Post.CustomFields["CustomClass"].Value;
    %>
    NeetTeen.CustomClass += '<%= CustomClass %>' + ' ';
    <% } if (Post.CustomFields.ContainsKey("CustomEnum")) {
        var CustomEnum = Post.CustomFields["CustomEnum"].Value;
    %>
    NeetTeen.CustomEnum += '<%= CustomEnum %>' + ' ';
    <% } %>
</script>
<% } %>
