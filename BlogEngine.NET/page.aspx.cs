...
public partial class page : BlogBasePage
{
    ...
    protected override void OnInit(EventArgs e)
    {
        ...
        base.SetPageTitle($"{Server.HtmlEncode(Page.Title)} - {BlogSettings.Instance.Name}");
        base.SetAbsoluteUrl(Page.AbsoluteLink.ToString());
        base.SetMetaDescription(Page.Description);
        AddMetaKeywords();
        if (Page.FirstImgSrc.Length > 0) {
            OgImage = Page.FirstImgSrc.Remove(0, 1); // remove starting '/'
        }
    }
    ...
    public string AdminLinks
    {
        get
        {
            ...
            if (this.Page.CanUserEdit)
            {
                ...
                sb.AppendFormat(
                    "<a href=\"{0}neet/app/editor/editpage.cshtml?id={1}\">{2}</a>",
