...
{
    protected override string OgType => "article";
        ...
        if ((!String.IsNullOrWhiteSpace(requestId)) && requestId.TryParse(out id)) {
            ...
            if (post != null) {
                if (!Page.IsPostBack && !Page.IsCallback && Request.RawUrl.Contains("?id=")) {
                    ...
                    base.SetPageTitle($"{encodedPostTitle} - {BlogSettings.Instance.Name}");
                    base.SetAbsoluteUrl(Post.AbsoluteLink.ToString());
                    base.SetMetaDescription(Post.Description);
                    AddMetaKeywords();
                    if (Post.FirstImgSrc.Length > 0) {
                        OgImage = Post.FirstImgSrc.Remove(0, 1); // remove starting '/'
                    }

                    base.AddMetaTag("author", Server.HtmlEncode(Post.AuthorProfile == null ? Post.Author : Post.AuthorProfile.FullName));
                    base.AddMetaProperty("article:tag", Server.HtmlEncode(string.Join(",", Post.Tags)));
                    base.AddMetaProperty("article:published_time", Post.DateCreated.ToString("s"));
                    base.AddMetaProperty("article:modified_time", Post.DateModified.ToString("s"));
                    base.AddMetaProperty("article:author", "100000346374602"); // Facebook profile Id
