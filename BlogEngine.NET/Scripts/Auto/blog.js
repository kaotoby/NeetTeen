...
BlogEngine = {
    ...
    showCommentPreview: function () {
        var oPreview = BlogEngine.$('preview');
        var oCompose = BlogEngine.$('compose');

        if (oPreview) oPreview.className = 'selected';
        if (oCompose) oCompose.className = '';
        BlogEngine.$('commentCompose').style.display = 'none';
        BlogEngine.$('commentPreview').style.display = 'block';
        BlogEngine.$('commentPreview').innerHTML = '<img src="' + BlogEngineRes.webRoot + 'Content/images/blog/ajax-loader.gif" width="24" height="24" alt="Loading" />';
        var argument = BlogEngine.$('commentPreview').innerHTML;
        BlogEngine.addComment(true);
        return false;
    }
	,
    composeComment: function () {
        ...
        if (BlogEngine.$('commentPreview')) {
            BlogEngine.$('commentPreview').style.display = 'none';
        }
        if (BlogEngine.$('commentCompose')) {
            BlogEngine.$('commentCompose').style.display = 'block';
        }
        return false;
    }
	,
    ...
    onCommentError: function (error, context) {
        ...
        if (BlogEngine.$('recaptcha_response_field')) {
            Recaptcha.reload();
        }
        if (BlogEngine.$("spnSimpleCaptchaIncorrect")) BlogEngine.$("spnSimpleCaptchaIncorrect").style.display = "none";
    ...
    addComment: function (preview) {
        ...
        if (!isPreview) {
            ...
            BlogEngine.$("status").innerHTML = BlogEngineRes.i18n.savingTheComment;
        }
        ...
        // var recaptchaResponseField = BlogEngine.$('g-recaptcha-response');
        var recaptchaResponse = "";
        if (typeof grecaptcha !== "undefined") {
            recaptchaResponse = grecaptcha.getResponse();
        }

        var simpleCaptchaChallengeField = BlogEngine.$('simpleCaptchaValue');
        ...
        var argument = author + "-|-" + email + "-|-" + website + "-|-" + country + "-|-" + content + "-|-" + notify + "-|-" + isPreview + "-|-" + captcha + "-|-" + replyToId + "-|-" + avatar + "-|-" + recaptchaResponse + "-|-" + simpleCaptchaChallenge;
    ...
    appendComment: function (args, context) {
        if (context == "comment") {
            ...
            if (args == "RecaptchaIncorrect" || args == "SimpleCaptchaIncorrect") {
                if (BlogEngine.$("spnCaptchaIncorrect")) BlogEngine.$("spnCaptchaIncorrect").style.display = "";
                if (BlogEngine.$("spnSimpleCaptchaIncorrect")) BlogEngine.$("spnSimpleCaptchaIncorrect").style.display = "";
                BlogEngine.toggleCommentSavingIndicators(false);
            }
            else if (args == "EmailNotValid") {
                ...
                if (BlogEngine.$("spnCaptchaIncorrect")) BlogEngine.$("spnCaptchaIncorrect").style.display = "none";
                if (BlogEngine.$("spnSimpleCaptchaIncorrect")) BlogEngine.$("spnSimpleCaptchaIncorrect").style.display = "none";
    ...
    validateAndSubmitCommentForm: function () {
        ...
        if (reg.test(BlogEngine.comments.emailBox.value) == false) {
            BlogEngine.$("status").innerHTML = "Email is not valid.";
            BlogEngine.$("status").className = "warning";
            BlogEngine.$("txtEmail").focus();
            return false;
        }
        ...
        //BlogEngine.$('spnNameRequired').style.display = bNameIsValid ? 'none' : '';
        ...
        //    bAuthorNameIsValid = !BlogEngine.equal(author, visitor);
        //}

        //BlogEngine.$('spnChooseOtherName').style.display = bAuthorNameIsValid ? 'none' : '';
    ...
    search: function (root, searchfield) {
        ...
        var input = BlogEngine.$(searchfield);
        var check = BlogEngine.$("searchcomments");
    ...
    rate: function (blogId, id, rating) {
        BlogEngine.createCallback("rating.axd?id=" + id + "&rating=" + rating, BlogEngine.ratingCallback, blogId);
    }
	,
    ...
    showRating: function (container, id, raters, rating, blogId) {
        ...
        for (var i = 1; i <= 5; i++) {
            ...
            a.innerHTML = i;
            a.href = 'rate/' + i;
            a.className = BlogEngine.englishNumber(i);
            a.title = BlogEngineRes.i18n.rateThisXStars.replace('{0}', i.toString()).replace('{1}', i == 1 ? '' : 's');
            a.rel = "nofollow";
            a.onclick = function () {
                BlogEngine.rate(blogId, id, BlogEngine.innerHTML);
                return false;
            };
